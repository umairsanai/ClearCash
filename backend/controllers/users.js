...
export const getWeeklySpendings = handleAsyncError(async (req, res, next) => {
    ...
    const spendings = await fetchSpendings(req.user.user_id, start_date, end_date);
    res.status(200).json({
        status: "success",
        data: spendings
    });    
...
export const transferMoneyToAnotherPocket = handleAsyncError(async (req, res, next) => {
    ...
    const updatedPockets = await fetchAllPockets(req.user.user_id);
...
async function fetchSpendings(user_id, start_date, end_date) {
...
export async function fetchCurrentMonthTransactions(user_id) {
